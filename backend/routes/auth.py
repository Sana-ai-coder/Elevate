...
def signup():
    ...
    try:
        user = User(name=name, email=email, password_hash=hash_password(password), grade=grade, role=role, is_verified=False)
        db.session.add(user)
        db.session.commit()
        current_app.logger.info(f"New user registered: {email} as {role}")
        token = create_access_token(user)
        return jsonify({"token": token, "user": {"id": user.id, "name": user.name, "email": user.email, "grade": user.grade, "role": user.role, "is_verified": user.is_verified}}), 201
...
def login():
    ...
    token = create_access_token(user)
    return jsonify({"token": token, "user": {"id": user.id, "name": user.name, "email": user.email, "grade": user.grade, "role": user.role, "is_verified": user.is_verified}})
...
def me():
    ...
    user = g.current_user
    return jsonify({"user": {"id": user.id, "name": user.name, "email": user.email, "grade": user.grade, "role": user.role, "is_verified": user.is_verified, "created_at": user.created_at.isoformat() if user.created_at else None}}), 200
