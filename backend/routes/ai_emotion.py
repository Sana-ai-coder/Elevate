...
Output:    JSON {
             "emotion"     : "happy",
             "confidence"  : 0.9124,
             "all_scores"  : {"angry":0.01, "confused":0.05, "happy":0.91, "neutral":0.03},
             "model_info"  : { ... }
           }
...
LEGACY_CLASS_NAMES = ["angry", "confused", "happy", "neutral"]
IMG_SIZE    = (96, 96)
...
def _align_scores_to_taxonomy(raw_probs: np.ndarray) -> dict:
    """Map model output probabilities to project taxonomy order."""
    probs = np.asarray(raw_probs, dtype=np.float32).reshape(-1)

    if probs.shape[0] == len(CLASS_NAMES):
        vals = probs
    elif probs.shape[0] == len(LEGACY_CLASS_NAMES):
        legacy = {name: float(probs[i]) for i, name in enumerate(LEGACY_CLASS_NAMES)}
        ...
        bored = 0.65 * neutral + 0.35 * confused
        focused = 0.60 * neutral + 0.40 * happy
        vals = np.array([happy, bored, focused, confused, neutral, angry], dtype=np.float32)
    else:
...
def predict_emotion():
    ...
    Response JSON:
        {
          "emotion"    : "happy",
          "confidence" : 0.9124,
          "all_scores" : {"angry": 0.01, "confused": 0.05,
                          "happy": 0.91, "neutral": 0.03},
    ...
    try:
        ...
        all_scores = _align_scores_to_taxonomy(probs)
